...
export default function EmployeeJobAssignment() {
  const { user } = useAuth();
  const [assignments, setAssignments] = useState<JobAssignmentUI[]>([]);
  ...
  // Fetch job assignments from backend
  const fetchAssignments = async () => {
    try {
      setLoading(true);
      setError(null);

      const params = user?.id ? { employeeId: user.id } : undefined;
      const assignRecs = await listEmployeeJobAssignments(params);

      const rows: JobAssignmentUI[] = assignRecs.map(rec => {
        const employeeName = rec.employee ? `${rec.employee.firstName ?? ''} ${rec.employee.lastName ?? ''}`.trim() || rec.employee.email : rec.employeeId;
        const jobTitle = rec.job?.title ?? rec.jobId;
        return {
          id: rec.id,
  ...
  return (
    <div className="h-full overflow-auto p-6">
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white/90">My Job Assignments</h1>
          <p className="mt-1 text-gray-600 dark:text-gray-400">View your current job assignments</p>
        </div>
      </div>
      ...
      {!loading && !error && filteredAssignments.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {searchTerm ? 'Try adjusting your search' : 'You have no job assignments yet'}
          </p>
        </div>
      )}
    </div>
